...
public class partitionsAndN {
        ...
        public static class MapperClass extends Mapper<LongWritable, Text, Text, helperMap> {
            ...
            private static Pattern ENGLISH_PATTERN = Pattern.compile("^[a-zA-Z]+.*$");
            // private static Pattern HEBREW_PATTERN = Pattern.compile("^[א-ת]+.*$"); // Used this to test on smaller Corpus
            @Override
            ...
            public void map(LongWritable lineId, Text line, Context context) throws IOException,  InterruptedException {
                ...
                Matcher m = ENGLISH_PATTERN.matcher(split_line[0]);
                if (!m.matches() || split_line[0].split(" ").length != 3 ) return; //checking for hebrew words in the trigram
                for (String word : split_line[0].split(" ")){
                    if (word.length() < 2 || (stopWords.contains(word))) return; //checking for stop words in the trigram
                }
